...
from imgclas_lo import paths, config, utils
...
def create_model_lo(CONF):
    ...
    x = GlobalAveragePooling2D()(x)
    print(x)
    #x = models.Model(inputs=base_model.output, outputs=x)
    # Tras el GlobalAveragePooling ya tenemos un stack denso, es donde corresponde añadir las layers de localizaciones
    # x = Flatten()(x) #might work better on large dataset than GlobalAveragePooling 
    # CONF['model']['num_locations']
    # https://github.com/keras-team/keras/issues/8470
    input_locations = Input(shape=(CONF['model']['num_locations'],))
    y=input_locations
    y= models.Model(inputs=input_locations, outputs=y)
    #combined = layers.concatenate([x.output, y.output])
    combined = layers.concatenate([x, y.output])
    z = Dense(2000,
    ...
    # Full model
    model = Model(inputs=[base_model.input, y.input], outputs=predictions)
