...
from imgclas_lo.data_utils import k_crop_data_sequence, k_crop_data_sequence_lo
...
def predict_lo(model, X, locations, num_locations, conf, top_K=None, crop_num=10, filemode='local', merge=False, use_multiprocessing=False):
    """
    Predict function.
    ...
    X : str or list
        List of images paths of length N. If providing a list of urls, be sure to set correctly the 'filemode' parameter.
        If a str is provided it will be understood as a single image to predict.
    locations: lista de etiquetas de localizacion correspondientes a cada imagen
    conf: dict
        Configuration parameters. The data augmentation parameters that will be used in the inference can be changed in
        conf['augmentation']['val_mode'].
    top_k : int
    ...
    data_gen = k_crop_data_sequence_lo(inputs=X,
                                    im_size=conf['model']['image_size'],
                                    locations = locations,
