# Método para generar la matriz de confusión EN EL CASO DE QUE ESTÉS TRABAJANDO CON IMGCLAS_LO
# Incluye un modo para trabajar con y otro sin locations sin más que responder True o False en el Input correspondiente. 
...
from imgclas_lo.data_utils import load_data_splits, compute_meanRGB, compute_classweights, load_class_names, load_location_names
from imgclas_lo import paths, config, model_utils, utils
from imgclas_lo.data_utils import load_image, load_data_splits, load_class_names, load_location_names
from imgclas_lo.test_utils import predict, predict_lo
from imgclas_lo import paths, plot_utils, utils, test_utils
from imgclas_lo.optimizers import customAdam

# User parameters to set
#Para saber si usamos location o no, no podemos usar bool(input...) ya que python reconoce todo string no vacío como True. Definimos la siguiente función:
#def use_location():
...
if use_lo:
    ...
    pred_lab, pred_prob = predict_lo(model = model, X = X, locations = locations, num_locations = num_locations, conf = conf, top_K=TOP_K, filemode='local')
...
top3 = test_utils.topK_accuracy(true_lab, pred_lab, K=3)
top4 = test_utils.topK_accuracy(true_lab, pred_lab, K=4)
...
print('Top3 accuracy: {:.1f} %'.format(top3 * 100))
m.write('Top3 accuracy: {:.1f} %'.format(top3 * 100) + '\n')
print('Top4 accuracy: {:.1f} %'.format(top4 * 100))
m.write('Top4 accuracy: {:.1f} %'.format(top4 * 100) + '\n')
